...
const LINKEDIN = 'https://www.linkedin.com/company/78576520'
...
export default function Page() {
  ...
  /* idioma: recupera preferência e mantém <html lang> em dia */
  useEffect(() => {
    const saved = window.localStorage.getItem('junebox-lang') as Lang | null
  ...
  const changeLang = useCallback((next: Lang) => {
    setLang(next)
    window.localStorage.setItem('junebox-lang', next)
    setMenu(false)
  }, [])
  ...
  const LangSwitch = ({ className = '' }: { className?: string }) => (
    <div className={`lang-switch ${className}`} role="group" aria-label="Idioma">
      {LANGS.map((l) => (
        <button
          key={l}
          type="button"
